...
void calc_uniform_mean_curvature (const std::vector<glm::vec3> &vertices, const std::vector< std::vector<unsigned short>> &triangles, std::vector<glm::vec3> &Lu) {
    
    std::vector<std::vector<unsigned short>> one_ring;
    collect_one_ring(vertices, triangles, one_ring);
    
    for(int i=0; i < vertices.size(); i++){
        std::vector<unsigned short> currentRing = one_ring[i];
        glm::vec3 sum(0.,0.,0.);
        
        for(unsigned short j : currentRing){
            sum += vertices[j];
        }
        
        sum /= currentRing.size();
        sum -= vertices[i];
...
void uniform_smooth(unsigned int _iters, std::vector<glm::vec3> &vertices, const std::vector< std::vector<unsigned short> > &triangles){
    for(int j=0; j < _iters; j++){
        
        std::vector<glm::vec3> Lu;
        calc_uniform_mean_curvature(vertices, triangles, Lu);
...
//    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    ...
    GLuint programID = LoadShaders( "../TP3_lissage/vertex_shader.glsl", "../TP3_lissage/fragment_shader.glsl" );
    ...
    // Load the texture
    GLuint Texture = loadDDS("../TP3_lissage/uvmap.DDS");
    ...
    std::string filename("../TP3_lissage/suzanne.off");
    ...
    for(auto indexed_vertice : indexed_vertices){
        bb_min = glm::min(bb_min, indexed_vertice);
        bb_max = glm::max(bb_max, indexed_vertice);
    }
    ...
    //Application du lissage sur les triangles du maillage
    uniform_smooth(5 , indexed_vertices, triangles);
    ...
    for(unsigned int valence : valences){
